...
with app.app_context():
    ...
    if not User.query.filter(User.email.like('admin@hacktm.ro')).first():
        user = User(name='admin',email='admin@hacktm.ro',password=generate_password_hash('admin'),active=True,admin=True)
        db.session.add(user)
        db.session.commit()
...
app.add_url_rule('/request_code', methods=['GET', 'POST'], view_func=request_code)
app.add_url_rule('/reset_password', methods=['GET', 'POST'], view_func=reset_password)
